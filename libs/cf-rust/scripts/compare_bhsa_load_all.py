...
def main() -> int:
    ...
    cf_rust_root = repo_root / "libs/cf-rust"
    ...
    rust = measure_command(
        [
            str(cf_rust_root / "target/release/cf_rust_bhsa_load_all"),
            str(args.tf_path),
            str(args.limit),
        ],
        cwd=cf_rust_root,
