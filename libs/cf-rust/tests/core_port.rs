...
use cf_rust::LogicalRange::{Range, Single};
use cf_rust::compiled::{
...
fn public_config_constants_match_python_config_api_values() {
    ...
    assert_eq!(BANNER, "This is Context-Fabric 0.5.0");
...
fn public_types_module_aliases_match_python_types_surface() {
    let node: cf_rust::types::Node = 7;
    let feature_value: cf_rust::types::FeatureValue = cf_rust::types::FeatureValue::string("typed");
    let slot_range: cf_rust::types::SlotRange = (1, node);
    let node_array: cf_rust::types::NodeArray = vec![1, 2, node];
    let index_array: cf_rust::types::IndexArray = vec![0, 1, 2];
    let offset_array: cf_rust::types::OffsetArray = vec![0, 3, 6];
    let search_result: cf_rust::types::SearchResult = vec![node];
    let section_spec: cf_rust::types::SectionSpec = vec!["book".to_string(), "chapter".to_string()];

    let mut metadata: cf_rust::types::MetaData = BTreeMap::new();
    metadata.insert("valueType".to_string(), Some("str".to_string()));
    let mut feature_metadata: cf_rust::types::FeatureMetaData = BTreeMap::new();
    feature_metadata.insert("word".to_string(), metadata.clone());

    let mut node_feature_data: cf_rust::types::NodeFeatureData = BTreeMap::new();
    node_feature_data.insert(node, FeatureValue::string("hello"));

    let mut edge_feature_data: cf_rust::types::EdgeFeatureData = BTreeMap::new();
    edge_feature_data.insert(node, BTreeSet::from([1, 2, 3]));
    let mut edge_feature_value_data: cf_rust::types::EdgeFeatureValueData = BTreeMap::new();
    edge_feature_value_data.insert(
        node,
        BTreeMap::from([(1, FeatureValue::string("contains"))]),
    );

    let mut nodes_by_type: cf_rust::types::NodesByType = BTreeMap::new();
...
fn public_downloader_helpers_match_python_registry_and_path_behaviors() {
    ...
    let request =
        cf_rust::build_download_request("researcher/cfabric-my-corpus", Some("v1"), true, true)
            .unwrap();
    ...
    let request =
        cf_rust::build_download_request("researcher/cfabric-my-corpus", None, false, false)
            .unwrap();
...
fn public_precompute_helpers_match_corpus_levels_order_and_rank() {
    ...
    let levels = cf_rust::precompute::levels(
        &node_types,
        &slot_sets,
        corpus.max_slot(),
        corpus.slot_type(),
    ...
    let order = cf_rust::precompute::order(
        &node_types,
        &slot_sets,
        &levels,
        corpus.max_slot(),
    ...
    let rank = cf_rust::precompute::rank(corpus.max_node(), &order);
    assert_eq!(rank, corpus.rank());
    let rank_computed = RankComputed::new(rank.clone());
    assert_eq!(&*rank_computed, &rank);

    let lev_up =
        cf_rust::precompute::lev_up(&slot_sets, &rank, corpus.max_slot(), corpus.max_node());
    assert_eq!(
        cf_rust::precompute::levUp(&slot_sets, &rank, corpus.max_slot(), corpus.max_node()),
        lev_up
    );
    ...
    let lev_down =
        cf_rust::precompute::lev_down(&lev_up, &rank, corpus.max_slot(), corpus.max_node());
    assert_eq!(
        cf_rust::precompute::levDown(&lev_up, &rank, corpus.max_slot(), corpus.max_node()),
        lev_down
    );
    assert_eq!(
    ...
    let boundary = cf_rust::precompute::boundary(&slot_sets, &rank, corpus.max_slot());
    assert_eq!(boundary, corpus.boundary());
    assert_eq!(boundary.first_slots[0], vec![6, 8]);
    assert_eq!(boundary.last_slots[4], vec![8, 7]);

    let sections = cf_rust::precompute::sections(&corpus).unwrap();
    assert_eq!(
        cf_rust::precompute::sectionsFromApi(&corpus).unwrap(),
        sections
    );
    ...
    assert!(cf_rust::precompute::structure(&corpus).is_none());
    ...
    let structure = cf_rust::precompute::structure(&structured).unwrap();
    ...
    let character_counts = cf_rust::precompute::characters(
        &BTreeMap::from([
            (
                "combo".to_string(),
...
fn mapped_compiled_string_pool_features_read_values_without_materializing() {
    ...
    let word_rows: Vec<_> = word.rows().collect::<cf_rust::Result<_>>().unwrap();
...
fn mapped_corpus_description_matches_materialized_description_without_materializing_cache() {
    ...
    assert_eq!(
        cf_rust::CorpusDescription::from_mapped(&mapped, &text, "mini").unwrap(),
