...
use crate::feature::{EdgeFeature, FeatureValue, NodeFeature, TfFeature, parse_value};
    ...
    I: IntoIterator<Item = Result<(usize, String)>>,
{
    ...
    for line_result in data_lines {
        ...
        let (sources, targets, raw_value) = if fields.len() >= 2 {
            ...
            if has_edge_values {
                let entries = parse_valued_edge_targets(fields[0], value_type, path, line_no)?;
                let targets = entries
                    .iter()
                    .map(|(target, _)| *target)
                    .collect::<Vec<_>>();
                for (target, value) in entries {
                    if let Some(value) = value {
                        edge_values.insert((implicit_node, target), value);
                    }
                }
                (vec![implicit_node], targets, None)
            } else {
                (
                    vec![implicit_node],
                    parse_node_spec(fields[0], path, line_no)?,
...
fn parse_valued_edge_targets(
    raw: &str,
    value_type: &str,
    path: &Path,
    line: usize,
) -> Result<Vec<(u32, Option<FeatureValue>)>> {
    let mut rows = Vec::new();
    for part in raw.split(',') {
        if let Some((target, raw_value)) = part.split_once(':') {
            for target in parse_node_spec(target, path, line)? {
                rows.push((target, parse_value(raw_value, value_type, path, line)?));
            }
        } else {
            for target in parse_node_spec(part, path, line)? {
                rows.push((target, None));
            }
        }
    }
    Ok(rows)
