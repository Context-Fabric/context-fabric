...
use context_fabric_core::compiled::{MappedCompiledCorpus, compile_features, inspect_compiled};
...
fn main() -> Result<()> {
    ...
    let cache_path = std::env::args()
        ...
        .map(PathBuf::from)
        .unwrap_or_else(|| {
            PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("target/bhsa-mapped-curated.cfr")
        });
    ...
    let compile_ms = if cache_has_required_features(&cache_path) {
        None
    } else {
        let compile_start = Instant::now();
        compile_features(&tf_path, &cache_path, FEATURES)?;
    ...
    println!(
        "loaded mapped_features={} compile_ms={} load_ms={:.3} nodes={}",
        FEATURES.len(),
        compile_ms
    ...
    for (id, template) in QUERIES {
        ...
        match search.search(template, Some(limit)) {
            Ok(results) => {
                println!(
...
fn cache_has_required_features(cache_path: &PathBuf) -> bool {
    let Ok(metadata) = inspect_compiled(cache_path) else {
        return false;
    };
    FEATURES.iter().all(|feature| {
        metadata.node_feature(feature).is_some() || metadata.edge_feature(feature).is_some()
    })
}
