#!/usr/bin/env python3
"""Compare implementation probe output against Text-Fabric golden masters."""
...
PROBE_DIR = ROOT / "libs" / "core" / "tests" / "golden" / "probes"
GOLDEN_DIR = ROOT / "libs" / "core" / "tests" / "golden" / "golden"
...
def run_python_dump(tf_path: Path, probes: Path) -> subprocess.CompletedProcess[str]:
    code = r"""
import json
import sys
from pathlib import Path
...
from gen_golden import normalize, run_probe

tf_path = Path(sys.argv[1])
probes = Path(sys.argv[2])
...
for line in probes.read_text().splitlines():
    ...
    result = normalize(run_probe(api, probe))
    print(json.dumps({"id": probe["id"], "result": result}, ensure_ascii=False))
"""
    return subprocess.run(
        [sys.executable, "-c", code, str(tf_path), str(probes)],
...
def compare_mode(name: str, mode: str, expected_by_id: dict[str, Any], tf_path: Path, probes: Path) -> bool:
    if mode == "python":
        completed = run_python_dump(tf_path, probes)
    else:
...
def compare(name: str, modes: list[str]) -> bool:
    ...
    ok = True
    for mode in modes:
        ok = compare_mode(name, mode, expected_by_id, tf_path, probes) and ok
    return ok
...
def main() -> None:
    ...
    parser.add_argument(
        "--mode",
        action="append",
        choices=["materialized", "mapped", "python"],
        default=None,
        help="Implementation mode to compare. May be repeated. Defaults to all modes.",
    )
    args = parser.parse_args()
    modes = args.mode or ["materialized", "mapped", "python"]

    ok = True
    for name in args.corpus:
        ok = compare(name, modes) and ok
