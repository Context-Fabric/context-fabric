...
\subsubsection{Worker Scaling}

\textbf{Text-Fabric} in spawn mode loads independent copies ($\sim$1.9 GB/worker), reaching 190 GB at 100 workers. Fork mode is nearly flat at $\sim$6.3 GB total (COW sharing), but requires that 6 GB minimum regardless of worker count.

\textbf{Context-Fabric} in spawn mode uses $\sim$315 MB/worker (32 GB at 100 workers). Fork mode starts at 305 MB with $\sim$23 MB per additional worker (2.6 GB at 100 workers).

\subsubsection{Multi-Corpus Scaling}

A more compelling scenario for memory-mapped architectures is serving \emph{multiple corpora simultaneously}---for instance, an API that provides access to Hebrew, Greek, Syriac, and Coptic texts, or different versions of the same corpus.

With Text-Fabric, each corpus must be loaded entirely into RAM:
\begin{itemize}
    \item 1 corpus: 6.3 GB
    \item 2 corpora: 12.6 GB
    \item 3 corpora: 18.9 GB (exceeds 16 GB VM)
\end{itemize}

With Context-Fabric, corpora share the memory-mapping infrastructure and only actively-accessed pages consume RAM:
\begin{itemize}
    \item 1 corpus: 305 MB
    \item 2 corpora: $\sim$610 MB
    \item 10 corpora: $\sim$3 GB
    \item 50 corpora: $\sim$15 GB
\end{itemize}

This represents a \textbf{20$\times$ improvement} in corpus density. A single 16 GB server running Context-Fabric can serve dozens of corpora that would require dedicated machines under Text-Fabric.

\begin{table}[h]
\centering
\begin{tabular}{lrrrr}
\toprule
\textbf{Workers} & \textbf{TF Spawn} & \textbf{CF Spawn} & \textbf{TF Fork} & \textbf{CF Fork}\footnote{CF fork mode has $\sim$23 MB per-worker overhead vs.\ near-zero for TF fork. At $\sim$260 workers, the two converge; beyond that TF fork uses less total memory. However, deployments exceeding 100 workers per machine are uncommon in practice.} \\
\midrule
1 & 6.3 GB & 305 MB & 6.3 GB & 305 MB \\
4 & 7.7 GB & 1.3 GB & 6.3 GB & 398 MB \\
10 & 19 GB & 3.2 GB & 6.4 GB & 535 MB \\
100 & 190 GB & 32 GB & 6.5 GB & 2.6 GB \\
\bottomrule
\end{tabular}
\caption{Projected memory consumption by worker count (up to 100 workers)}
\label{tab:scaling}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{TF fork mode}: 2 effective workers (6 GB base leaves limited headroom)
    \item \textbf{CF spawn mode}: $\sim$50 workers
    \item \textbf{CF fork mode}: $\sim$50 workers (at 305 MB + 23 MB each)
\end{itemize}

Within realistic deployment scales (up to $\sim$100 workers), CF maintains a significant advantage. The practical implications:
