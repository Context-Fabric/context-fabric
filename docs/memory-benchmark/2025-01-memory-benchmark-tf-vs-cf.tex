...
Context-Fabric addresses this challenge through a memory-mapped architecture that achieves 79--94\% memory reduction while maintaining compatibility with Text-Fabric's Python API (the \texttt{F}, \texttt{L}, \texttt{T}, \texttt{S} interfaces researchers use in their code). This paper quantifies these efficiency gains through controlled benchmarking.
...
\begin{itemize}
    \item 16$\times$ faster load times improve interactive experience
    \item 94\% less memory enables work on modest hardware (laptops, older machines)
    \item Same corpus, same API---no workflow changes required
\end{itemize}

The trade-off---a larger cache (387 MB vs. 138 MB)---is minor. Context-Fabric compiles faster than Text-Fabric (30s vs. 62s) because it passes pre-computed data directly to the compiler, avoiding redundant parsing. Every subsequent session benefits from 16$\times$ faster loads and 94\% lower memory consumption.
...
Given the above considerations, we measure memory consumption using RSS. While RSS double-counts shared pages in multi-process scenarios, this ``worst case'' view is more honest than USS for memory-mapped workloads.

An earlier version of this benchmark used USS, which produced misleading results:

\begin{itemize}
    \item USS excluded memory-mapped pages backed by files, even when those pages consumed physical RAM
    \item This made Context-Fabric appear to use near-zero memory in fork scenarios
    \item The 94\% efficiency claim was technically accurate for USS but obscured the true memory footprint
\end{itemize}

We chose RSS for transparency: the numbers represent actual memory pressure on the system, even if this is less flattering to our memory-mapped architecture.
...
Four independent worker processes, each loading the corpus from scratch. This simulates \texttt{multiprocessing} with \texttt{spawn} context, common on macOS and Windows where fork is unavailable or discouraged.
...
\midrule
Memory (RSS) & 5.2 GB & 300 MB & 94\% \\
Load Time & 8.1 s & 0.51 s & 16$\times$ faster \\
Cache Size & 138 MB & 387 MB & 2.8$\times$ larger \\
\bottomrule
\end{tabular}
...
Context-Fabric achieves 94\% memory reduction in single-process mode. The larger cache size reflects the uncompressed numpy array format, which trades disk space for memory-mapping capability.
...
\textbf{Metric} & \textbf{Text-Fabric} & \textbf{Context-Fabric} & \textbf{Reduction} \\
\midrule
Total Memory & 6.0 GB & 1.2 GB & 79\% \\
Per Worker & 1.5 GB & 310 MB & 4.9$\times$ less \\
\bottomrule
\end{tabular}
...
In spawn mode, each worker loads independently. Context-Fabric's per-worker footprint (310 MB) reflects the memory-mapped approach where only accessed pages reside in physical memory.

The Text-Fabric total (6.0 GB) is less than $4 \times$ single-process (5.2 GB) due to shared library pages and measurement timing---workers were measured after loading but before extensive feature access.
...
\midrule
Single process & 5.2 GB & 300 MB & 94\% \\
4 workers (spawn) & 6.0 GB & 1.2 GB & 79\% \\
4 workers (fork) & 6.3 GB & 397 MB & 94\% \\
\bottomrule
\end{tabular}
...
\caption{Performance comparison between Text-Fabric and Context-Fabric on the BHSA corpus. Top left: cache load time (16$\times$ faster). Top right: single-process memory usage (94\% reduction). Bottom left: spawn mode per-worker memory (4.9$\times$ less). Bottom right: fork mode per-worker memory (16$\times$ less).}
...
Context-Fabric's memory efficiency comes with minimal trade-offs:

\begin{itemize}
    \item \textbf{Larger cache size}: Uncompressed numpy arrays require $\sim$2.8$\times$ more disk space (387 MB vs. 138 MB for BHSA).
    \item \textbf{I/O sensitivity}: Performance depends on storage speed; SSDs recommended for optimal results.
\end{itemize}

Notably, compilation is actually \emph{faster} with Context-Fabric (30s vs. 62s) because it passes pre-computed data directly to the compiler, avoiding redundant parsing of source files. Every subsequent session---whether by a production API serving thousands of requests or a researcher running interactive queries---benefits from:

\begin{itemize}
    \item 16$\times$ faster load times (0.51s vs. 8.1s)
    \item 94\% less memory (300 MB vs. 5.2 GB)
    \item Ability to run on modest hardware (laptops, cloud free tiers)
\end{itemize}

For individual researchers, this means: compile once on first use, then enjoy dramatically improved performance for all future work. The sub-second loads save time on every session.

\subsection{Fork Mode Considerations}

The fork mode results deserve careful interpretation:

\begin{enumerate}
    \item \textbf{RSS double-counting}: Our methodology sums main + worker RSS, which double-counts shared COW pages. The ``true'' physical memory is lower than reported.
    \item \textbf{COW page faults}: As workers modify data (triggering copy-on-write), memory consumption increases. Our benchmark accesses features read-only, representing best-case sharing.
    \item \textbf{Platform availability}: Fork mode is discouraged on macOS (due to framework issues) and unavailable on Windows. Spawn mode results are more portable.
\end{enumerate}

\subsection{Scaling Analysis}

Our benchmarks measured 4 workers. How do these results extrapolate to larger deployments?

\subsubsection{Text-Fabric Scaling}

In spawn mode, each Text-Fabric worker loads an independent copy of the corpus:

\begin{itemize}
    \item Memory per worker: $\sim$1.5 GB
    \item 10 workers: 15 GB
    \item 100 workers: 150 GB
    \item 1000 workers: 1.5 TB (impractical on any single machine)
\end{itemize}

In fork mode with preloading, memory is dominated by the parent process:

\begin{itemize}
    \item Base memory: 6.3 GB (parent process)
    \item Additional per worker: $\sim$14 MB (COW overhead)
    \item Scaling is nearly flat, but requires 6+ GB minimum
\end{itemize}

\subsubsection{Context-Fabric Scaling}

In spawn mode, Context-Fabric workers share memory-mapped pages:

\begin{itemize}
    \item Memory per worker: $\sim$310 MB
    \item 10 workers: 3.1 GB
    \item 100 workers: 31 GB
    \item 1000 workers: 310 GB
\end{itemize}

In fork mode, the base footprint is minimal:

\begin{itemize}
    \item Base memory: 305 MB (parent process)
    \item Additional per worker: $\sim$23 MB
    \item 10 workers: 305 + 230 = 535 MB
    \item 100 workers: 305 + 2,300 = 2.6 GB
    \item 1000 workers: 305 + 23,000 = 23 GB
\end{itemize}
...
\begin{tabular}{lrrrr}
\toprule
\textbf{Workers} & \textbf{TF Spawn} & \textbf{TF Fork} & \textbf{CF Spawn} & \textbf{CF Fork} \\
\midrule
1 & 5.2 GB & 6.3 GB & 300 MB & 305 MB \\
4 & 6.0 GB & 6.3 GB & 1.2 GB & 397 MB \\
10 & 15 GB & 6.4 GB & 3.1 GB & 535 MB \\
100 & 150 GB & 6.5 GB & 31 GB & 2.6 GB \\
1000 & 1.5 TB & 6.5 GB & 310 GB & 23 GB \\
\bottomrule
\end{tabular}
\caption{Projected memory consumption by worker count}
\label{tab:scaling}
\end{table}

\subsection{Implications for Production Deployments}

For a typical cloud VM with 16 GB RAM:

\begin{itemize}
    \item \textbf{TF spawn mode}: Maximum 2--3 workers
    \item \textbf{TF fork mode}: 1 effective worker (6 GB leaves little headroom)
    \item \textbf{CF spawn mode}: $\sim$50 workers
    \item \textbf{CF fork mode}: $\sim$600 workers
\end{itemize}

This represents a \textbf{200$\times$ improvement} in worker density for fork mode deployments. The practical implications:

\begin{itemize}
    \item \textbf{Lower infrastructure costs}: Fewer and smaller instances needed
    \item \textbf{Higher throughput per dollar}: More concurrent requests per GB of RAM
    \item \textbf{Modest hardware viability}: Corpus APIs can run on free-tier cloud instances or personal computers
    \item \textbf{AI agent scalability}: Multiple concurrent AI conversations can query the same corpus without resource contention
\end{itemize}
...
This benchmark demonstrates that Context-Fabric's memory-mapped architecture achieves 79--94\% memory reduction compared to Text-Fabric across all tested deployment scenarios. The efficiency gains are most pronounced in fork-based production deployments, where Context-Fabric's 397 MB footprint (vs. 6.3 GB for Text-Fabric) enables a 200$\times$ improvement in worker density.
...
Source code is available at \url{https://github.com/codykingham/context-fabric}.
