...
\begin{abstract}
For nearly ten years, Text-Fabric (TF) has served as a powerful tool for the analysis of classical corpora.
It has propelled an entire generation of linguistic and exegetical researchers. However, TF has
always been burdened by two problems: 1) it is resource intensive, consuming large amounts of RAM,
and 2) it requires knowledge of Python or the query language to use it. These frictions have
limited TF's usefulness to the philological community, which is not known for its computational prowess!
Agentic AI, however, now makes it possible to solve the skill-gap. In order to do so, TF must become light and agile,
capable of running on resource-constrained systems, with the ability to scale to many corpora at once.
...
(today the Eep Talstra Centre for Bible and Computer, ETCBC) with the purpose of using
computers to analyze the Hebrew Bible.
Over the next fifty years, those efforts yielded a rich and advanced grammatical database
of Biblical Hebrew. For most of this period, the dataset was kept privately in the Werkgroep's
internal server. It was not until 2014 that Dirk Roorda, a researcher on the [†?] project,
published the dataset online as an XML dump in the Linguistic Annotation Framework (LAF) [†cite].
LAF modeled texts as graphs, with the various linguistic units like clauses and phrases being captured
...
of walking the LAF nodes and edges and querying their features [†Roorda]. This package carried Biblical
Hebrew research into the ecosystem of Data Science and Machine Learning, which had begun to embrace Python as the
default scripting language. LAF-Fabric was soon picked up by enthusiasic researchers, who used
it for a host of statistical analyses [† Synvar, Kalkan, Kingham]. In 2016, Roorda improved again
on the idea by releasing Text-Fabric [†Roorda].

which depended on a stand-off data model rather than in-line XML. This 
