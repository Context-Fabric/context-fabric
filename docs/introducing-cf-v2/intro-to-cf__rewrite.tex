...
of Biblical Hebrew. For most of this period, the dataset was kept privately on the Werkgroep's
...
published the dataset online as an XML dump in the Linguistic Annotation Framework (LAF) [†cite].
LAF modeled texts as graphs. The various linguistic units like clauses and phrases were captured
as nodes with features, and their relations as edges [†cite https://link.springer.com/article/10.1007/s10579-014-9268-1].
Alongside the XML dump, Roorda published a Python package called LAF-Fabric, which had an API capable
of walking the nodes and edges to query their features [†Roorda]. By choosing Python, Roorda 
...
then analyze them with tools like Numpy, Pandas, Scikit Learn, and others [†cite].
These new capabilities were soon picked up by enthusiasic researchers who leveraged them for a whole host
of studies, placing new emphasis on data-driven, reproducible results [† Synvar, Kalkan, Kingham, Naaijer, Jacobs, Erwich,
...
each \texttt{.tf} file contributed a single node feature or edge relationship. So, if one only wanted to analyze
...
The \texttt{.tf} format was a single column of data, with each row indexed by its node identifier.
This dramatically reduced both storage requirements and corpus load times because it stripped away the
syntactic overhead required by the XML format. Roorda envisioned that this new nimble format,
along with a host of user-friendly visualization tools, would catalyze a new category of scholar,
the ``programming theologian''.
...
most likely, come to separate the eras of manually-written code and machine-generated code.
...
its \textit{Claude Code} tool. Here was a tool which could generate entire packages in one go, complete
with unit tests and documentation. The ongoing expansion of this trend with tools like
\textit{Codex} and \texit{Grok Build} leave little doubt that software engineering will never be the same.

The AI revolution now makes it possible for humanities scholars to close the technical skill gap,
potentially furthering Roorda's vision of scientific transformation. To maximize this opportunity,
several transformations to Text-Fabric are required. Firstly, corpus search must become \textit{deployable}
to a production environment. This means the ability to serve search from a persistent web server for
...
especially under high traffic loads, this is undesirable. Memory mapping offers an alternative approach
where the graph data could be stored on disk and accessed only when needed. This method is already used
by a number of performance-sensitive machine learning tools like Pytorch and Tensorflow. Secondly,
endpoints have to be designed that can return the information agents need to navigate and search various
corpora. Corpus description should ideally be generated dynamically, so that the endpoints work out-of-the-box
with any corpus. This involves giving the agent a way to query available nodes, edges, and features along with
illustrative examples. For search, Text-Fabric already has a template-based query language. By giving the agent
a syntax guide, they can dynamically compose queries which address the user's needs.

guide agents in the navigation of a given corpus 

Non-technical users will not have development environments installed on their
personal computers. And while AI agents can now create these environments, they also .

Because we want to move coding responsibility from the researcher to an AI agent, we need to 


Fabric should be made capable of
serving 
