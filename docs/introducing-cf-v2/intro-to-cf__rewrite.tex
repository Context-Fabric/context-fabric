...
\begin{abstract}
For nearly ten years, Text-Fabric (TF) has served an entire generation of linguistic and exegetical
researchers as a powerful tool for corpus analysis. However, TF has always been burdened by two problems:
1) it is resource intensive, consuming large amounts of RAM, and 2) it requires knowledge of Python or
the query language to use it. These frictions have limited its usefulness to the philological community,
...
It made it easy to, within the same program, collect statistics on a given grammatical pattern and
...
with a stand-off plain-text format, the \texttt{.tf} file. It made the text graph modular: each \texttt{.tf}
file contributed a single feature annotation or edge relationship. So, if one only wanted to analyze
...
along with a host of user-friendly visualization tools, would catalyze a whole new category of scholar,
the ``programming theologian''.

The vision of the theologian who programs was certainly ambitious, and it dovetailed well with
...
Such was the state of things until 2022, when OpenAI announced on Twitter its release of a new
public demo of its Large Language Model, ChatGPT [†cite].
